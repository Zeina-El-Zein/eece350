...
class PithonClient:
    ...
    def _draw_game(self):
        ...
        if gs:
            ...
            # use each player's chosen color for their own name
            c1 = self.snake_color if self.my_player_id == 1 else (80, 140, 255)
            c2 = self.snake_color if self.my_player_id == 2 else (60, 200, 120)

            self._text(f"{s1.get('username','P1')}  HP:{s1.get('health',0)}",
                self.font_med, c1, 10, 15)
            self._text(f"{m:02d}:{s:02d}", self.font_big, C_YELLOW,
                GRID_W * CELL_SIZE // 2, 10, center=True)
            self._text(f"{s2.get('username','P2')}  HP:{s2.get('health',0)}",
                self.font_med, c2, GRID_W * CELL_SIZE - 10, 15, right=True)
        ...
        for key, obs_type in gs.get("obstacles", {}).items():
            ...
            col  = OBSTACLE_COLORS.get(obs_type, C_GRAY)
            pygame.draw.rect(self.screen, col,
                (x * CELL_SIZE + 2, TOP + y * CELL_SIZE + 2,
        ...
        for key, pie_type in gs.get("pies", {}).items():
            ...
            cy   = TOP + y * CELL_SIZE + CELL_SIZE // 2
            pygame.draw.circle(self.screen, col, (cx, cy), CELL_SIZE // 2 - 2)

        # snakes — player 1 uses chosen color, player 2 uses their chosen color
        ...
        if self.game_state:
            ...
            for snake_key, pid in [("snake1", 1), ("snake2", 2)]:
                ...
                col   = self.snake_color if pid == self.my_player_id else \
                        (80, 140, 255) if self.my_player_id == 1 else (60, 200, 120)
                y += 25
                self._text(uname, self.font_tiny, col, sx, y)
                y += 16
